import React, { forwardRef } from "react";

export interface InputProps extends React.InputHTMLAttributes<HTMLInputElement> {
  label?: React.ReactNode;
  error?: string;
  helperText?: string;
  startIcon?: React.ReactNode;
  endIcon?: React.ReactNode;
}

const Input = forwardRef<HTMLInputElement, InputProps>(
  ({ className = "", type = "text", label, error, helperText, startIcon, endIcon, ...props }, ref) => {
    const baseClasses = "w-full px-4 py-3 bg-white border border-zinc-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-zinc-900/10 focus:border-zinc-900 transition-all font-['Google_Sans'] placeholder:text-zinc-400";
    const startIconClasses = startIcon ? "pl-10" : "";
    const endIconClasses = endIcon ? "pr-10" : "";
    const errorClasses = error ? "border-red-300 focus:ring-red-100 focus:border-red-500" : "";
    const combinedClasses = `${baseClasses} ${startIconClasses} ${endIconClasses} ${errorClasses} ${className}`.trim();

    return (
      <div className="space-y-2">
        {label && (
          <label className="text-sm font-medium text-zinc-700 font-['Google_Sans']">
            {label}
          </label>
        )}
        <div className="relative">
          {startIcon && (
            <div className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-400">
              {startIcon}
            </div>
          )}
          <input
            type={type}
            className={combinedClasses}
            ref={ref}
            {...props}
          />
          {endIcon && (
            <div className="absolute right-3 top-1/2 -translate-y-1/2 text-zinc-400">
              {endIcon}
            </div>
          )}
        </div>
        {error && (
          <p className="text-sm text-red-600 font-['Google_Sans']">{error}</p>
        )}
        {helperText && !error && (
          <p className="text-sm text-zinc-500 font-['Google_Sans']">{helperText}</p>
        )}
      </div>
    );
  }
);

Input.displayName = "Input";

export { Input };